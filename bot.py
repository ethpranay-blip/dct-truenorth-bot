...
def schedule_sessions():
    ...
    for sess, times in offsets.items():
        ph, pm = times["pre"]
        scheduler.add_job(send_session_brief, "cron", hour=ph, minute=pm, args=[sess, "pre"], id=f"pre_{sess}", replace_existing=True)
        posth, postm = times["post"]
        scheduler.add_job(send_session_brief, "cron", hour=posth, minute=postm, args=[sess, "post"], id=f"post_{sess}", replace_existing=True)
    for i, (h, m) in enumerate([(5,15), (13,15), (18,15)]):
        scheduler.add_job(send_regime_update, "cron", hour=h, minute=m, args=["scheduled"], id=f"regime_{i}", replace_existing=True)
@bot.event
async def on_ready():
    ...
    if cid == CH["claude"]:
        async with message.channel.typing():
            ...
            if cid not in chat_history:
                chat_history[cid] = []
            finance_kw = ["$","btc","bitcoin","eth","sol","crypto","trade","market","stock","forex","gold","oil","chart","level","support","resistance","breakout","trend","regime","macro","setup","long","short","entry","tp","sl","bias","outlook"]
            is_finance = any(k in user_msg.lower() for k in finance_kw)
            if is_finance:
                tn_data = await query_truenorth(user_msg)
                system = "You are a professional trading analyst. TrueNorth provided the market data below. Synthesize into a clear actionable response. Use $SYMBOL format."
                reply = query_claude([{"role": "user", "content": f"User asked: {user_msg}\\n\\nTrueNorth data:\\n{tn_data}"}], system=system)
                ...
                reply = query_claude(chat_history[cid], system="You are a helpful assistant in a crypto trading Discord. Be concise.")
