...
_tn_access_token: str = os.getenv("TN_TOKEN", "")
                            ...
                            async with httpx.AsyncClient(timeout=30) as client:
                                        ...
                                        if resp.status_code == 200:
                                                        ...
                                                        if new_token:
                                                                            ...
                                                                            print(f"[TokenRefresh] ✅ Token refreshed successfully at {datetime.now(IST).isoformat()}")
                                                                            return True
                                        else:
                print(f"[TokenRefresh] ⚠️ Refresh response had no token field. Keys: {list(data.keys())}")
else:
            print(f"[TokenRefresh] ❌ Refresh failed — HTTP {resp.status_code}: {resp.text[:200]}")
except Exception as e:
        print(f"[TokenRefresh] ❌ Exception during refresh: {e}")
    return False


async def query_truenorth(prompt):
        headers = {
                    "Authorization": f"Bearer {_tn_access_token}",
                    "Content-Type": "application/json",
