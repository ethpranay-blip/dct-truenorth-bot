...
- **Auto-signals (`auto_signal_scan`, 4h UTC cron at :10, only if `CH_SIGNALS` set)** — the `!setup` engine run unattended with a stricter gate. Pipeline: `detect_regime` (skip on UNKNOWN) → `performance_scanner` (universe 15, full ranked list) → top-3 leaders as LONG candidates (unless RISK-OFF) + bottom-3 laggards as SHORT (unless RISK-ON) → per candidate: 4h TA score via `build_rule_setup(min_score=4.0, tp_rs=(2.5, 4.0))` **and** RVWAP structural bias (`compute_rvwaps`: rolling Σ(close·vol)/Σvol over 7/30/90/365 daily bars from `historical_bars`; `rvwap_bias`: price above both 365d & 90d ⇒ LONG, below both ⇒ SHORT) — signal posts only when score direction, RVWAP bias, and regime all agree. Dedupe reuses `_find_recent_open`; trades log with `source: "auto"` (vs `"manual"`), and `compute_winrate` reports a per-engine W/L line once any auto trade exists. A second leg (`_scan_watchlist`) runs `SIGNALS_WATCHLIST` (env defaults) **plus the user watchlist** (`_WATCHLIST`, curated via `!watch`/`!unwatch`, persisted to `watchlist.json`, admin-gated, capped at 25) — deduped, each evaluated by `_eval_watchlist_symbol` against RVWAP bias + score ≥ 4.0 (crypto scores on 4h TA with daily bars fetched for the RVWAP read; stocks/commodities on daily). No BTC regime — RVWAP structural bias is the trend filter there.

## Dashboard feed — `/api/live`

`build_live_payload` serves the companion dashboard's live panels: `open_setups` (from in-memory `_SETUPS`, each carrying a `last_price` stamped by `track_setups` every 15 min), `crossasset` (a Skew-equivalent lead-lag map — `build_cross_asset_map` composes 7d %-moves for crypto majors/stocks/commodities from `historical_bars` + 24h index/rates/VIX from `market_index_price`, grouped into tiles with a one-line lead/lag read; cached and recomputed every 15 min by the `dashboard_cache` job), and `brief` (the latest session-brief text, stashed on post). `stocks` adds a screener over `STOCKS_SCREENER` benchmarked vs QQQ (one batched daily-bars call → 24h/30d %, 30d RS vs Nasdaq-100 with 21 trading bars ≈ one month, RVWAP bias, daily signal score, 30-close spark; recomputed in the same 15-min cache job). All keyless; the endpoint reads cache/memory so it stays fast. This mirrors the strategy stored in the TN app agent's memory — if the weights/thresholds change here, change them there.
